...
constexpr bool equal(double a, double b, double margin = 1e-10) { return std::abs((a - b)) < margin; }
...
TEST_CASE("Test util functions") {
    ...
    SECTION("Test StringMaker") {
        auto convert = [](auto container) { return StringMaker<decltype(container)>::convert(container); };
